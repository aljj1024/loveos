...
const defaultState: AppState = {
  ...
  mood: { current: DEFAULT_MOOD, setAt: new Date().toISOString() },
};
...
type AppAction =
  ...
  | { type: 'CLAIM_WISHLIST_ITEM'; id: string; claimedBy: 'husband' | 'wife' }
  ...
  switch (action.type) {
    ...
    case 'ACCEPT_TASK': {
      const tasks = state.tasks.map(t =>
        t.id === action.taskId
          ? { ...t, status: 'accepted' as const, acceptedBy: 'husband' as const, acceptedAt: new Date().toISOString() }
          : t
      );
      return { ...state, tasks };
    }
    ...
    case 'SET_MOOD':
      return { ...state, mood: { current: action.mood, setAt: new Date().toISOString() } };

    case 'RESET_MOOD':
      return { ...state, mood: { current: DEFAULT_MOOD, setAt: new Date().toISOString() } };
...
// Convenience: dispatch + toast helper
